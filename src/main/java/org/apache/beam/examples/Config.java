	...
	public static Config create(boolean streaming) {
		...
		switch (runner != null ? runner : "") {
			case "direct": {
				...
				DirectOptions pipelineOptions =
						PipelineOptionsFactory.create().as(DirectOptions.class);
				pipelineOptions.setRunner(DirectRunner.class);
				builder.setPipelineOptions(pipelineOptions).setCassandraHosts("localhost")
						.setCassandraPort(9042).setCassandraKeyspace("test")
						.setCassandraTable1("table1").setCassandraTable2("table2")
						.setKafkaUrls("localhost:9092").setKafkaTopic("test_topic")
						.setKafkaConsumerGroupId("test_grp_1").setoutputPath("./output");
				break;
			}
			case "flink-local": {
			...
			case "flink-cluster": {
				...
				pipelineOptions
						.setFilesToStage(Arrays.asList("./build/libs/beam-playground-0.1.jar"));
				pipelineOptions.setParallelism(3);
				pipelineOptions.setTempLocation("/tmp");
				// pipelineOptions.setStreaming(streaming);
				builder.setPipelineOptions(pipelineOptions)
						.setCassandraHosts("cassandra.default.svc.cluster.local")
						.setCassandraPort(9042).setCassandraKeyspace("test")
						.setCassandraTable1("table1").setCassandraTable2("table2")
						.setKafkaUrls("kafka-broker.default.svc.cluster.local:9092")
						.setKafkaTopic("test_topic").setKafkaConsumerGroupId("test_grp_1")
						.setoutputPath("/tmp");
				break;
			}
			// case "spark": {
